...
if __name__ == "__main__":
    ...
    for k in range(len(angles)):
        ...
        endeffector = PointAtAngleAndDistance(50,state*360)
        cv2.imshow('frame',cv2.circle(cv2.line(np.zeros((300,300)),(150,150),(150+int(endeffector[0]),150+int(endeffector[1])),(0.5),2),(150+int(endeffector[0]),150+int(endeffector[1])),5,(1),-1))
        if(first):
            cv2.waitKey(0)
            first = False
        cv2.waitKey(int(step*1000))
    cv2.waitKey(500)